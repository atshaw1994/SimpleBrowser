...
{
    public class JsonHistoryItemRepository : IHistoryRepository
    {
        private readonly string _storageFolder;
        private readonly JsonSerializerOptions _jsonOptions;

        public JsonHistoryItemRepository()
        {
            _storageFolder = Path.Combine(
                Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
                "SimpleBrowser\\History\\"
            );
        ...
        public async Task SaveAsync(HistoryItemModel item)
        {
            ArgumentNullException.ThrowIfNull(item);

            // Create a safe, fixed-length hash of the URL
            byte[] hashBytes = MD5.HashData(Encoding.UTF8.GetBytes(item.DateVisited.ToString("yyyyMMddHHmmss") + item.Url));
        ...
        public Task DeleteAsync(HistoryItemModel item)
        {
            ArgumentNullException.ThrowIfNull(item);
        ...
        public async Task<IEnumerable<HistoryItemModel>> LoadAllAsync()
        {
